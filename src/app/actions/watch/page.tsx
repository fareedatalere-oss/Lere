...
export default function WatchPage() {
  ...
  useEffect(() => {
    // Real Adsterra Banner Injection
    const script = document.createElement("script");
    script.type = "text/javascript";
    script.src = "//pl25841234.highperformanceformat.com/23/45/67/23456789.js"; // This would be your real Adsterra script source
    script.async = true;
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-md mx-auto space-y-6">
        ...
        {/* Real Adsterra Banner Container */}
        <div id="ad-container" className="w-full bg-slate-100 rounded-xl min-h-[100px] flex items-center justify-center border-2 border-dashed border-slate-300 overflow-hidden">
          <div className="flex flex-col items-center justify-center p-4 text-center">
            <MonitorPlay className="h-6 w-6 text-slate-400 mb-2" />
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Premium Ad Placement</p>
          </div>
        </div>
        ...
        <Card className="border-none shadow-xl overflow-hidden bg-slate-900 rounded-3xl aspect-video flex flex-col items-center justify-center p-8 text-center">
          ...
          <h2 className="text-white font-bold text-xl">Earn ₦2.50</h2>
          <p className="text-white/60 text-xs mt-2 mb-6">Interact with the network to claim your payout.</p>
          <Button 
            className="w-full h-14 bg-primary hover:bg-primary/90 rounded-2xl text-white font-bold shadow-lg"
            onClick={handleClaim}
            disabled={isClaiming}
          >
            {isClaiming ? "Processing..." : "Claim Reward Now"}
          </Button>
        </Card>
        ...
        >
          ...
          Open Smart Link: {SMART_LINK_ID}
        </Button>

        <div className="p-4 bg-primary/5 rounded-2xl border border-dashed border-primary/20 flex items-center gap-3">
          <ShieldCheck className="h-5 w-5 text-primary" />
          <p className="text-[10px] font-bold uppercase text-primary leading-tight">Secured by Adsterra Network</p>
        </div>
